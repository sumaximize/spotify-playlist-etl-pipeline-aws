...
def lambda_handler(event, context):
    ...
    for file in s3.list_objects(Bucket=Bucket, Prefix=Key)["Contents"]:
        file_key = file["Key"]      # raw-data/to-processed/
    ...
    for data in spotify_data:
        ...
        # Write album, song, artist data to bucket store
        song_key = f"transformed_data/song_data/song_transformed_{str(datetime.now())}.csv"
        song_buffer = StringIO()
        song_df.to_csv(song_buffer, index=False)
        song_content = song_buffer.getvalue()
        s3.put_object(Bucket=Bucket, Key=song_key, Body=song_content)
        
        album_key = f"transformed_data/album_data/album_transformed_{str(datetime.now())}.csv"
        album_buffer = StringIO()
        album_df.to_csv(album_buffer, index=False)
        album_content = album_buffer.getvalue()
        s3.put_object(Bucket=Bucket, Key=album_key, Body=album_content)
        
        artist_key = f"transformed_data/artist_data/artist_transformed_{str(datetime.now())}.csv"
